...
<?php 
  require ('sql_connect.inc');
    //sql_connect('blog');
    $stmt = $conn->prepare("SELECT `name` FROM time_unit ");
    $stmt->execute();
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-content-wrapper">
          <div class="container-fluid">
            <form class="form-horizontal" role="form" method="post" action="<?php echo 'insert_schedule.php?' ?>">
      ...
      <select class="form-control" id="statement" name="statement">  
       <?php
        while ($res = $stmt->fetch()) {
          echo '<option>'.$res['id_statement'].'</option>';
        }
        $conn = null;
       ?>
      </select>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-10">
        <select class="form-control" id="setting" name="setting" onchange="fetch_setting(this.value);">
          ...
          <?php
            $idx=0;
            while ($idx<sizeof($units)) {
              echo '<option>'.$units[$idx].'</option>';
              $idx++;
            }
          ?>
        </select>
  ...
  <div class="form-group">
    <label class="control-label col-sm-2" for="tipe">Weekdays</label>
      <div class="col-sm-3">
        <input type="text" class="form-control" id="weekday" name="weekday" required>
      </div>
      <div class="col-sm-7">
